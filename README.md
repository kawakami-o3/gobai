...
```sh
make install                  # 依存をインストール (bun install)
make dev                      # フル Tauri dev (Vite + Rust + WebView、動的ポート選択)

bun run dev                   # Frontend 単独 (Vite dev サーバのみ)
make check-rust               # Rust 単独のコンパイル確認 (cargo check)
```

利用可能な make ターゲットは `make help` で一覧表示できる。
